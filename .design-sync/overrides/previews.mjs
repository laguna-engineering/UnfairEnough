...
// forked from design-sync lib/previews.mjs — inject a PREVIEW-ONLY banner that
// renames react-native-web's injected `<style id="react-native-stylesheet">`.
// That element's id starts with "r" and its rules live in CSSOM (empty
// innerHTML), so package-validate's `#root, [id^="r"]` root selector picks it
// first and false-flags every RN-web card as `[RENDER] root empty`. Renaming
// it (harmless — RNW holds the element by reference) lets the real cell roots
// (r0/r1/…) be measured. This banner lands ONLY in _preview/*.js, never in the
// uploaded _ds_bundle.js, so designs built with the DS are untouched.
import { build } from 'esbuild';
import { createHash } from 'node:crypto';
import { IIFE_IMPORT_META_DEFINE, hypothesisLine } from '../../.ds-sync/lib/common.mjs';

const RNW_STYLESHEET_ID_FIX =
  "(function(){if(typeof document==='undefined')return;" +
  "var fix=function(){var e=document.getElementById('react-native-stylesheet');if(e)e.id='ds-rnw-sheet';};" +
  "fix();try{new MutationObserver(fix).observe(document.documentElement,{childList:true,subtree:true});}catch(_){}" +
  "})();";
  ...
  for (const c of components) {
    ...
    try {
      await build({
        ...
        banner: { js: RNW_STYLESHEET_ID_FIX },
