// forked from design-sync lib/bundle.mjs — React Native web resolution
// (alias react-native → react-native-web, .web.* extensions, .js-as-JSX,
// browser conditions) so @unfairenough/ui's RN components bundle for the web.
// ONLY sharedBuildOptions' esbuild resolve config is changed; the output
// contract (stampHeader header, IIFE/window footer) is byte-for-byte upstream.
//
...
function sharedBuildOptions({ nodePaths, tsconfig }) {
  ...
  return {
    ...
    plugins,
    metafile: true,
    // ── React Native web resolution (fork addition) ──
    // Expo/RN on web = react-native-web + platform-suffixed entry points.
    // esbuild does none of this by default, so the RN primitives, the
    // expo-linear-gradient web build, and react-native-svg must be steered here.
    ...
    loader: {
      // expo-linear-gradient (and other RN web packages) ship JSX inside .js.
      '.js': 'jsx',
      '.svg': 'dataurl',
      '.png': 'dataurl',
      '.woff': 'dataurl',
    ...
    minify: false,
    // __DEV__ is an RN/React convention; unset it crashes RN web modules.
